...
# Docker env vars are only available to the entrypoint (PID 1). SSH sessions
# started by sshd don't inherit them. Write the vars that matter to
# /etc/profile.d/devbox.sh so all login shells pick them up.
{
    echo "# Generated by devbox entrypoint"
    [[ -n "${ATUIN_SYNC_ADDRESS:-}" ]] && echo "export ATUIN_SYNC_ADDRESS=\"${ATUIN_SYNC_ADDRESS}\""
